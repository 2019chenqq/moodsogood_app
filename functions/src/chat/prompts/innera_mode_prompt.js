...
const MODE_RULES = Object.freeze({
  ...
  recentReview: [
    ...
    "recentReviewSummary 是統計與數值的唯一依據；recentReviewEvidence 只能用來列舉實際日期或例子，不得用 evidence 重新統計，也不得因最多 5 筆 evidence 就推論只有 5 天有資料。使用者詢問日期時只能引用 evidence 中確實存在的日期，不得捏造未提供的日期或事件。",
    ...
    "睡眠 duration 優先使用 recentReviewSummary.sleep 的 averageNightSleepHours、averageAllDaySleepHours、averageNapHours、shortestNightSleepHours、longestNightSleepHours，並以小時呈現；一般情況不要改寫成分鐘。",
