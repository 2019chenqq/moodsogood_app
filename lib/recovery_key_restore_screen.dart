...
class _RecoveryKeyRestoreScreenState extends State<RecoveryKeyRestoreScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _verify() async {
    ...
    try {
      ...
      final aesKey =
          await SecureStorageService.recoverOriginalAesKeyFromRecoveryKey(
      ...
      if (mounted) {
        final msg = switch (e.toString()) {
          String s when s.contains('尚未設定備援金鑰') => '此帳號尚未設定備援金鑰，無法使用此方式還原。',
          String s when s.contains('找不到加密資料') => '找不到帳號的加密資料，請確認是否登入正確帳號。',
        ...
        title: const Row(
          children: [
            Icon(Icons.check_circle_rounded,
                color: Color(0xFF73E5BE), size: 24),
            SizedBox(width: 8),
            Text(
              '備援金鑰驗證成功',
                            ...
                            Expanded(
                              child: ClipRRect(
                                ...
                                child: LinearProgressIndicator(
                                  ...
                                  backgroundColor: const Color(0x335FB8D9),
                                  valueColor:
