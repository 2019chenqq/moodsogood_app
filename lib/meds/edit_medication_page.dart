...
// ✅ 藥物形式：口服 / 長效針
    ...
    _scheduleType = (d['scheduleType'] as String?) ?? 'daily';
    final scheduleInterval = d['scheduleIntervalDays'];
    if (scheduleInterval is num && scheduleInterval >= 2) {
      _scheduleIntervalDays = scheduleInterval.toInt().clamp(2, 30).toInt();
    }
    _scheduleWeekdays.addAll(
      (d['weekdays'] as List? ?? const [])
          .whereType<num>()
...
// 若是長效針：通常不需要 times（避免混進早上/睡前）
    ...
    if (dosePerUnitVal is int) {
      _dose = dosePerUnitVal.toDouble();
    } else if (dosePerUnitVal is double) {
      _dose = dosePerUnitVal;
    } else if (doseVal is int) {
      _dose = doseVal.toDouble();
    } else if (doseVal is double) {
      _dose = doseVal;
    } else {
      _dose = 0;
    }
...
// ),
  ...
  Future<void> _editPillCountManually() async {
    ...
    if (result != null) {
      setState(() => _pillCount = result.clamp(0.1, 20));
    }
  }
